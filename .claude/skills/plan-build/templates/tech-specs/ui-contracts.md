...
- `{ComponentName}` — verbs: {list} | endpoint: `{METHOD} {/path}` (§API Contracts) | reuse: {path | new}
...
- **Server-connected components endpoint**: lookup in the UI ↔ API Traceability Matrix data **computed in memory earlier in Step 4** (joining API Contracts + UI Inventory Capability Join), though serialized later per canonical order. Build order: compute Matrix first (in memory) → serialize UI Contracts using in-memory Matrix lookups → serialize Matrix as the canonical-order-last subsection. The in-memory Matrix is the single source-of-truth for endpoint references; both UI Contracts (position 6) and serialized Matrix (position 7) consume the same computed data.
