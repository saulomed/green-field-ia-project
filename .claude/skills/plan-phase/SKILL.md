...
- Casos extremos ausentes, visíveis pela leitura da spec (ex: "login do usuário" sem menção ao que acontece quando o email não está confirmado). Reportar aqui **apenas** lacunas aparentes na leitura estática de um único documento. Casos extremos que só aparecem ao simular a execução — rastreando entradas, saídas derivadas, falhas parciais e estados — pertencem à "Consequências não mapeadas" abaixo, cujo trace por requisito é **obrigatório e complementar** a este check (não substitui nem é substituído por ele).
...
- Limites/quotas e TTLs/expirações descobertos pelo trace dinâmico (dimensões 8 e 10 do checklist de "Consequências não mapeadas") que não tenham valor concreto definido — tratá-los como decisão ausente a confirmar com o usuário, não inventar o valor silenciosamente.
...
Ao contrário das categorias acima — que comparam documentos entre si de forma estática — esta exige simular mentalmente a execução de cada requisito funcional e verificar se todas as consequências estão endereçadas.

**Disciplina obrigatória (trace por requisito).** Esta verificação se aplica a **cada requisito funcional da fase**, não apenas aos que parecem arriscados. Para cada requisito, percorrer o **checklist de dimensões** abaixo, considerando apenas as dimensões aplicáveis àquele requisito. Ao final de cada requisito, registrar uma nota curta — ou o(s) achado(s) encontrado(s), ou explicitamente `tracei — nada a reportar`. Esse registro é um artefato **interno** do processo de validação (apresentado ao usuário junto com os demais achados); **não** entra no documento de fase final. O objetivo não é enumerar exaustivamente todo edge case, mas garantir que **nenhum requisito passe sem o trace** e que consequências que nenhum documento endereça sejam sinalizadas.

**Checklist de dimensões.** Nem toda dimensão se aplica a todo requisito — pular as inaplicáveis, mas **considerar cada uma**. Os exemplos variam de domínio de propósito (CRUD, mídia, pagamentos, mensageria) para reforçar que as dimensões são universais, não específicas de auth:

1. **Valores-limite da entrada** — vazio, mínimo, máximo, overflow, unicode, comprimento extremo, zero/negativo, ausente vs. vazio. _Ex. (mídia): upload de arquivo de 0 byte, ou exatamente no limite de 10GB, ou com nome só de caracteres especiais._
2. **Saídas derivadas/geradas e restrições** — todo valor derivado ou gerado pode colidir, transbordar a coluna, violar unicidade/formato ou quebrar um consumidor downstream por uma entrada legítima? Rastrear cada valor até onde é armazenado ou usado. _Ex. (CRUD): slug derivado do título — dois títulos diferentes geram o mesmo slug, único na spec mas não na prática._
3. **Entidades relacionadas, cascata e deleção** — criar/alterar/excluir afeta quais dependentes diretos e transitivos? Há órfãos, efeitos em cascata ou ordem de remoção não especificados? _Ex. (mensageria): excluir um tópico que ainda tem mensagens pendentes na fila — elas somem, falham ou ficam órfãs?_
4. **Ciclo de vida / máquina de estados** — todas as transições de estado estão definidas? Há estados terminais? Transições ilegais são explicitamente rejeitadas? _Ex. (pagamentos): reembolsar um pagamento ainda não capturado, ou já estornado._
5. **Concorrência e condições de corrida** — duas requisições simultâneas na mesma operação produzem duplicação, lost update ou estado inconsistente? _Ex. (CRUD): duas criações simultâneas do mesmo recurso sujeito a restrição de unicidade._
6. **Idempotência e repetição** — reenvio ou retry (double-submit do cliente, retry de webhook, reprocessamento de job) produz efeito duplicado? _Ex. (pagamentos): o cliente reenvia o POST de cobrança após um timeout de rede._
7. **Caminho de falha e falha parcial entre sistemas** — uma operação que cruza limites (DB + email + storage + fila) não tem transação distribuída: se um passo falha após outro ter efetivado, há rollback, compensação ou limpeza? _Ex. (mídia): a linha do vídeo é gravada no DB, mas o upload ao storage falha em seguida._
8. **Temporal, ordenação e TTL** — expiração, clock skew, ordem de chegada de eventos assíncronos, limpeza de registros expirados. _Ex. (assíncrono): o evento "item processado" chega antes do evento "item criado"._
9. **Cardinalidade (zero / um / muitos)** — o requisito assume "um" mas e se houver zero relacionados? E milhares (paginação, limite de retorno, performance)? _Ex. (CRUD): listar os itens de uma conta recém-criada (zero) vs. uma com 100 mil itens._
10. **Limites, quotas e abuso** — há limite de tamanho, contagem ou taxa? O que acontece ao excedê-lo? _Ex. (mídia): um usuário inicia 1000 uploads simultâneos._
11. **Autorização e ownership** — checagem de propriedade do recurso, acesso cross-tenant, e privilégio em entidade relacionada (não só na raiz). _Ex. (CRUD): editar um comentário de outro usuário num recurso que eu possuo — quem pode?_
12. **Indisponibilidade de dependência externa** — se email/storage/fila/serviço externo estiver fora, qual o comportamento visível ao usuário? Há retry, enfileiramento ou falha explícita? _Ex. (assíncrono): o broker de mensagens está indisponível no momento do publish._

**Reportar cada achado** da mesma forma que outros problemas de validação: citar o requisito, descrever a consequência não mapeada e pedir ao usuário que decida. Adicionalmente, **classificar cada achado pela dimensão** que o originou (facilita o agrupamento — ver abaixo) e marcá-lo como **bloqueante** (impede gerar o plano até a decisão do usuário) ou **não-bloqueante** (há um padrão razoável a assumir; sinalizar via marcador `[DECIDIR: ...]` no rascunho em vez de parar).

**Se problemas forem encontrados:** parar e apresentá-los ao usuário antes de gerar o plano. Agrupar por tipo (inconsistência, ambiguidade, decisão ausente, lacuna de dependência, consequência não mapeada). Para as consequências não mapeadas, sub-rotular cada achado pela dimensão do checklist que o originou (ex: "consequência não mapeada — falha parcial", "consequência não mapeada — idempotência", "consequência não mapeada — ciclo de vida"). Ser específico — citar as declarações conflitantes ou apontar exatamente a capacidade que está pouco clara. Aguardar o usuário resolver. Depois que o usuário confirmar as resoluções (tipicamente atualizando `docs/decisions/` para a fase atual, o escopo da fase em `docs/project-plan.md`, ou fornecendo decisões explícitas de fora do escopo para consequências não mapeadas), reler as fontes afetadas e re-executar as verificações de validação antes de gerar o plano.
